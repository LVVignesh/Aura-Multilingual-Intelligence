...
class TextSummarizer:
    ...
    def get_summary(self, text, num_sentences):
        ...
        # If we only got 1-2 sentences but the text is very long (common in speech-to-text),
        # split it into chunks based on word count
        if len(sentences) <= 2 and len(text.split()) > 50:
            words_list = text.split()
            chunk_size = max(15, len(words_list) // (num_sentences * 2))  # Adaptive chunk size
            sentences = []
        ...
        # If still only one sentence and it's the entire text, return a portion of it
        if len(sentences) == 1 and len(text.split()) > 30:
            words = text.split()
            # Return first portion as summary
            summary_length = min(len(words) // 3, 50)
...
class AutomaticNotesMaker:
    def __init__(self):
        ...
        for package in ['punkt', 'stopwords', 'averaged_perceptron_tagger']:
            try:
                nltk.data.find(f'tokenizers/{package}' if package == 'punkt' else f'corpora/{package}' if package == 'stopwords' else f'taggers/{package}')
            except LookupError:
                try:
    ...
    def setup_variables(self):
        ...
        self.auto_translate = ctk.BooleanVar(value=True)  # Auto-translate enabled by default
        self.languages = {
    ...
    def setup_live_tab(self):
        ...
        self.live_lang_var = ctk.StringVar(value="Hindi")  # Default to Hindi
        ...
        self.auto_translate_check.pack(side="left", padx=10)
    ...
    def speak_text(self, text, lang):
        """Speak text in any language using Google TTS"""
        if not text.strip():
            return
        ...
        def speak():
            ...
            try:
                # Get language code
                lang_code = self.languages.get(lang, 'en')
                
                # Use gTTS for all languages (better multi-language support)
        ...
        with sr.Microphone() as source:
            ...
            while self.is_listening:
                try:
                    ...
                    # Auto-translate if enabled and target language is not English
                    if self.auto_translate.get() and self.live_lang_var.get() != "English":
                        self.update_status("Auto-translating...")
                        try:
                            target_lang = self.languages[self.live_lang_var.get()]
                            translated = self.translator.translate(text, src='auto', dest=target_lang)
                            self.live_translated.insert("end", translated.text + "\n")
                            self.live_translated.see("end")
                        except Exception as e:
    ...
    def translate_live_text(self):
        threading.Thread(target=self._translate_thread, args=(self.live_text, self.live_translated, self.live_lang_var.get()), daemon=True).start()

    def translate_file_text(self):
        threading.Thread(target=self._translate_thread, args=(self.file_text, self.file_translated, self.file_lang_var.get()), daemon=True).start()

    def _translate_thread(self, source_widget, target_widget, target_language):
        """Translate text in background thread"""
        text = source_widget.get("1.0", "end").strip()
        if text:
            try:
                # Determine which tab we're on
                tab = 'live' if source_widget == self.live_text else 'file'
                self.update_status("Translating...", tab=tab)
                
                target_lang = self.languages[target_language]
                
                # Force source language to be auto-detected and translate to target
                translated = self.translator.translate(text, src='auto', dest=target_lang)
                
                # Update UI on main thread
                self.root.after(0, lambda: target_widget.delete("1.0", "end"))
                self.root.after(0, lambda: target_widget.insert("1.0", translated.text))
                self.update_status("Translation complete", tab=tab)
                
            except Exception as e:
                error_msg = f"Translation error: {str(e)}\n\nTry:\n1. Check internet connection\n2. Reinstall googletrans: pip install googletrans==4.0.0-rc1"
                self.update_status(f"Translation failed: {str(e)}", True, tab=tab)
                self.root.after(0, lambda: CTkMessagebox(title="Translation Error", message=error_msg, icon="cancel"))
    
    def process_audio_file(self):
        file_path = ctk.filedialog.askopenfilename(filetypes=self.supported_formats)
        if file_path:
            # Run in background thread to keep UI responsive
            threading.Thread(target=self._process_audio_thread, args=(file_path,), daemon=True).start()
    ...
    def _process_audio_thread(self, file_path):
        ...
        try:
            ...
            if file_path.lower().endswith('.mp3'):
                try:
                    self.update_status("Converting MP3 to WAV...", tab='file')
            ...
            text = self._transcribe_audio_with_chunking(use_path)
            
            # Update UI on main thread
            self.root.after(0, lambda: self.file_text.delete("1.0", "end"))
            self.root.after(0, lambda: self.file_text.insert("1.0", text))
            self.update_status("Ready", tab='file')
            ...
            self.root.after(0, lambda: CTkMessagebox(title="Error", message=str(e), icon="cancel"))

    def _transcribe_audio_with_chunking(self, audio_path):
        """Transcribe audio file, chunking if necessary to avoid API limits"""
        # Load audio to check duration
        audio = AudioSegment.from_wav(audio_path)
        duration_ms = len(audio)
        chunk_length_ms = 60000  # 60 seconds per chunk (API limit is ~1 minute)
        
        # If audio is short, process directly
        ...
        for i in range(num_chunks):
            ...
            self.update_status(f"Processing chunk {i+1}/{num_chunks}...", tab='file')
            ...
            try:
                with sr.AudioFile(chunk_path) as source:
                    ...
                    try:
                        ...
                        full_text.append(f"[Chunk {i+1} error: {str(e)}]")
            finally:
                # Cleanup chunk file
        ...
        return " ".join(full_text) if full_text else "[No speech detected]"

    def clean_and_process(self):
        file_path = ctk.filedialog.askopenfilename(filetypes=self.supported_formats)
        if file_path:
            # Run in background thread
            threading.Thread(target=self._clean_and_process_thread, args=(file_path,), daemon=True).start()

    def _clean_and_process_thread(self, file_path):
        """Clean and process audio in background thread"""
        ...
        try:
            ...
            # Normalize audio
            self.update_status("Normalizing audio...", tab='file')
            ...
            # Process the cleaned audio with chunking
            text = self._transcribe_audio_with_chunking(wav_path)
